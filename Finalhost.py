...
PREMIUM_EMOJI = {
    ...
    "📭": "5352896944496728039", "🛑": "5413610645142642221", "🔢": "5226929552319594190",
...
def premium_html(text: str) -> str:
    # Premium entities are rendered only in message bodies; keyboards keep Unicode labels.
    for glyph, emoji_id in sorted(PREMIUM_EMOJI.items(), key=lambda x: len(x[0]), reverse=True):
...
class Runner:
    ...
    async def _run(self, job_id: str, project: sqlite3.Row, user_id: int, chat_id: int, mode: str, log_path: Path) -> None:
        async with self.semaphore:
            ...
            has_manifest = (project["kind"] == "python" and requirements.is_file()) or (project["kind"] == "node" and package_json.is_file())
            runtime_network = "bridge" if has_manifest else RUNNER_NETWORK
            requirements = project_path / "requirements.txt"
            package_json = project_path / "package.json"
            has_python_deps = project["kind"] == "python" and requirements.is_file()
            has_node_deps = project["kind"] == "node" and package_json.is_file()
            # A manifest needs registry access during install; ordinary projects retain the configured network policy.
            runtime_network = "bridge" if (has_python_deps or has_node_deps) else RUNNER_NETWORK
            docker = ["docker", "run", "--rm", "--name", container, "--init", "--read-only", "--cap-drop=ALL", "--security-opt=no-new-privileges", "--pids-limit", "128", "--memory", "512m", "--cpus", "1.0", "--network", runtime_network, "-v", f"{project_path}:/workspace:rw", "-w", "/workspace"]
            for key, value in env.items():
                docker.extend(["-e", f"{key}={value}"])

            if project["kind"] == "python":
                if not (project_path / project["entrypoint"]).is_file():
                    raise ValueError(f"Python entrypoint not found: {project['entrypoint']}")
                if has_python_deps:
                    command = ["sh", "-lc", f"python -m pip install --no-cache-dir -r {shlex.quote(requirements.name)} && python -u {shlex.quote(project['entrypoint'])}"]
                else:
                    command = ["python", "-u", project["entrypoint"]]
            elif project["kind"] == "node":
                if not (project_path / project["entrypoint"]).is_file():
                    raise ValueError(f"JavaScript entrypoint not found: {project['entrypoint']}")
                if has_node_deps:
                    command = ["sh", "-lc", f"npm install --omit=dev && node {shlex.quote(project['entrypoint'])}"]
                else:
            ...
            if mode == "service":
                if command[:2] == ["sh", "-lc"]:
                    command = ["sh", "-lc", command[2] + "; code=$?; while true; do sleep 2; " + command[2] + "; code=$?; done"]
                else:
                    command = ["sh", "-lc", "while true; do " + " ".join(command) + "; sleep 2; done"]
            ...
            try:
                ...
                status_icon = '✅' if result_code == 0 else '❌'
...
def custom_reply_button(label: str) -> KeyboardButton:
    ...
    if emoji_id:
        try:
            return KeyboardButton(text=label, icon_custom_emoji_id=emoji_id)
        except TypeError:
            pass
...
def bottom_menu() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup([
        [custom_reply_button("📦 Dependencies")],
        [custom_reply_button("⏰ Schedule")],
        [custom_reply_button("⏱️ Unschedule")],
        [custom_reply_button("👤 User Information")],
        [custom_reply_button("💽 Storage")],
        [custom_reply_button("🆕 New Project")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text('Menu', reply_markup=bottom_menu())
...
async def projects(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await target.reply_text("📂 Projects\n\nSelect a project:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def project_panel(query, project: sqlite3.Row) -> None:
    ...
    text = (f"📂 Project Details\n\n"
            ...
            f"Type: <code>{esc(project['kind'])}</code>\n\n"
            f"Entrypoint: <code>{esc(project['entrypoint'])}</code>\n\n"
            f"Status: {icon} <code>{esc(project['status'])}</code>")
...
async def run_command(update: Update, context: ContextTypes.DEFAULT_TYPE, mode: str = "once") -> None:
    if not guard(update):
        return
    if not context.args:
        await update.message.reply_text('⚠️ Usage: /run PROJECT_NAME')
        return
    ...
    await update.message.reply_text(f'⏳ <b>Job queued:</b> <code>{job_id}</code>', parse_mode=ParseMode.HTML)
...
async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not row and not is_admin(update.effective_user.id):
        ...
        return
    await update.message.reply_text(" Stop requested." if await runner.stop(job_id) else '⚠️ Job is not running.')
...
async def logs_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(f'📜 <b>Job Logs:</b>\n\n<pre>{esc(text)}</pre>', parse_mode=ParseMode.HTML)
...
async def delete_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text('🗑️ Project deleted successfully.')


async def env_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not guard(update) or len(context.args) < 3:
        await update.message.reply_text('⚠️ Usage: /env PROJECT KEY VALUE')
        return
    project = project_for(update.effective_user.id, context.args[0])
    key = context.args[1]
    if not project or not re.fullmatch(r"[A-Z_][A-Z0-9_]{0,63}", key):
        await update.message.reply_text('❌ Invalid project or environment key format.')
        return
    with db() as conn:
...
async def schedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not guard(update) or len(context.args) < 2:
        await update.message.reply_text('⚠️ Usage: /schedule PROJECT_NAME MINUTES')
        return
    project = project_for(update.effective_user.id, context.args[0])
    try:
    ...
    if not project or minutes < 1 or minutes > 10080:
        await update.message.reply_text('❌ Project not found or interval must be between 1–10080 minutes.')
        return
    with db() as conn:
        conn.execute("DELETE FROM schedules WHERE project_id=?", (project["id"],))
    ...
    await update.message.reply_text(f' Schedule enabled for <b>{esc(project["name"])}</b> every {minutes} minutes.', parse_mode=ParseMode.HTML)


async def unschedule_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not guard(update) or not context.args:
        await update.message.reply_text('⚠️ Usage: /unschedule PROJECT_NAME')
        return
    project = project_for(update.effective_user.id, context.args[0])
    if not project:
        await update.message.reply_text('❌ Project not found.')
    ...
    with db() as conn:
        conn.execute("DELETE FROM schedules WHERE project_id=?", (project["id"],))
    await update.message.reply_text(' Schedule removed successfully.')
...
async def myinfo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (f' <b>Account Information</b>\n\n'
            ...
            f'✅ Approved: <code>yes</code>\n\n'
            f'💽 Storage: <code>{row["used_mb"]:.2f} MB</code> / <code>{row["quota_mb"]} MB</code>')
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not lower.endswith((".py", ".js", ".zip")):
        await update.message.reply_text('❌ Only <code>.py</code>, <code>.js</code>, and <code>.zip</code> files are supported.', parse_mode=ParseMode.HTML)
        return
    if not document.file_size or document.file_size > MAX_FILE_MB * 1024 * 1024:
        await update.message.reply_text(f'⚠️ File is too large. Maximum allowed: {MAX_FILE_MB} MB.')
        return
    context.user_data["pending_upload"] = {"file_id": document.file_id, "name": name, "size": document.file_size}
    await update.message.reply_text('📨 File received! Now send a project name using:\n\n<code>/name PROJECT_NAME</code>', parse_mode=ParseMode.HTML)
...
async def name_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not pending or not context.args:
        await update.message.reply_text('⚠️ Please upload a file first, then use <code>/name PROJECT_NAME</code>', parse_mode=ParseMode.HTML)
        return
    ...
    with db() as conn:
        ...
        if count >= MAX_PROJECTS:
            await update.message.reply_text(f'⚠️ Project limit reached: {MAX_PROJECTS}')
            return
        if user["used_mb"] + pending["size"] / 1024**2 > user["quota_mb"]:
            await update.message.reply_text('⚠️ Storage quota exceeded.')
            return
    ...
    try:
        ...
        await update.message.reply_text(f'✅ <b>Project Created Successfully!</b>\n\n'
                                        f'• Name: <code>{esc(project_name)}</code>\n\n'
                                        f'• Type: <code>{kind}</code>\n\n'
                                        f'• Entrypoint: <code>{entry}</code>\n\n'
                                        f'▶️ Run project: /run {esc(project_name)}', parse_mode=ParseMode.HTML)
    except Exception as exc:
        shutil.rmtree(root, ignore_errors=True)
        await update.message.reply_text(f'❌ Upload rejected: <code>{esc(exc)}</code>', parse_mode=ParseMode.HTML)
...
async def request_access(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not admin_ids:
        await query.edit_message_text("Access request could not be sent: no administrator is configured.")
    ...
    if delivered:
        ...
        await query.edit_message_text("Access request could not be delivered. Ask the administrator to open this bot and press /start first.")
...
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(f'✅ Approved user ID: <code>{target}</code>', parse_mode=ParseMode.HTML)
...
async def remove_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if target in ADMIN_IDS:
        await update.message.reply_text('⚠️ Administrators cannot be removed.')
        return
    with db() as conn:
        conn.execute("UPDATE users SET approved=0 WHERE user_id=?", (target,))
    await update.message.reply_text(f'🚫 Access removed for user ID: <code>{target}</code>', parse_mode=ParseMode.HTML)
...
async def schedule_menu(query, user_id: int) -> None:
    ...
    await query.edit_message_text("⏰ Schedule\n\nProject ကိုရွေးပါ။", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def unschedule_menu(query, user_id: int) -> None:
    ...
    keyboard.append([custom_inline_button("⬅️ Back", callback_data="projects")])
    await query.edit_message_text("⏱️ Unschedule\n\n24/7 service ရပ်မည့် project ကိုရွေးပါ။", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_pending_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if rename:
        ...
        if new_name != text.strip() or not new_name:
            await update.message.reply_text("⚠️ Invalid name. Use letters, numbers, dot, dash, or underscore.")
            return True
        if rename["kind"] == "project":
            with db() as conn:
                ...
                if duplicate or not row:
                    await update.message.reply_text("❌ Project name already exists or project was not found.")
                    return True
                conn.execute("UPDATE projects SET name=?,updated_at=? WHERE id=?", (new_name, now(), rename["project_id"]))
            await update.message.reply_text(f"✏️ Project renamed to <b>{esc(new_name)}</b>.", parse_mode=ParseMode.HTML)
            return True
        old_path = Path(rename["path"])
        new_path = old_path.with_name(new_name)
        if new_path.exists():
            await update.message.reply_text("❌ That file name already exists.")
            return True
        try:
            old_path.rename(new_path)
            await update.message.reply_text(f"✏️ File renamed to <code>{esc(new_name)}</code>.", parse_mode=ParseMode.HTML)
        except Exception as exc:
            await update.message.reply_text(f"❌ Rename failed: <code>{esc(exc)}</code>", parse_mode=ParseMode.HTML)
        return True
    return False
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text == "📦 Dependencies":
        await update.message.reply_text("📦 Dependencies\n\nProject ထဲမှာ requirements.txt သို့မဟုတ် package.json ထည့်ပြီး Run နှိပ်ပါ။")
    elif text in ("⏰ Schedule", "⏱️ Unschedule"):
        rows = []
        with db() as conn:
            rows = conn.execute("SELECT id,name FROM projects WHERE user_id=? ORDER BY created_at DESC", (update.effective_user.id,)).fetchall()
        cb = "schedule_project:" if text == "⏰ Schedule" else "unschedule_project:"
        title = "⏰ Schedule" if text == "⏰ Schedule" else "⏱️ Unschedule"
        keyboard = [[custom_inline_button(f"{title.split()[0]} {row['name']}", callback_data=f"{cb}{row['id']}")] for row in rows]
        keyboard.append([custom_inline_button("⬅️ Back", callback_data="projects")])
        await update.message.reply_text(f"{title}\n\nProject ကိုရွေးပါ။", reply_markup=InlineKeyboardMarkup(keyboard))
    elif text == "👤 User Information":
...
async def file_panel(query, project_id: str, root: Path, relative_path: str) -> None:
    ...
    await query.edit_message_text(f"📄 <b>{esc(relative_path)}</b>", parse_mode=ParseMode.HTML, reply_markup=keyboard)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "request_access":
        ...
        await query.edit_message_text("⏰ Schedule\n\nRun interval ကိုရွေးပါ။", reply_markup=keyboard)
        ...
        await query.edit_message_text(f"⏰ Schedule enabled\n\nEvery {minutes} minutes.")
        ...
        await query.edit_message_text(premium_html("⏱️ 24/7 Service stopped."))
        ...
        try:
            path = file_from_token(Path(row["path"]), token)
            path.unlink()
            await query.edit_message_text(premium_html("🗑️ File deleted successfully."), reply_markup=InlineKeyboardMarkup([[custom_inline_button("⬅️ Back", callback_data=f"files:{project_id}")]]))
        except Exception:
            await query.edit_message_text("❌ File could not be deleted.")
        ...
        if row:
            ...
            await query.edit_message_text(premium_html("🗑️ Project deleted successfully."), reply_markup=menu())
        else:
            await query.edit_message_text("❌ Project not found.")
        ...
        if row:
            ...
            await query.edit_message_text(premium_html("📁 Files\n\nဖိုင်တစ်ခုရွေးပါ။"), reply_markup=InlineKeyboardMarkup(keyboard))
    elif ":" in data:
        ...
        if action == "run":
            ...
            with db() as conn:
                running = conn.execute("SELECT id FROM jobs WHERE project_id=? AND status='running' ORDER BY started_at DESC LIMIT 1", (project_id,)).fetchone()
            await query.edit_message_text("⏹️ Stop requested." if running and await runner.stop(running["id"]) else "Project is not running.")
        elif action == "logs":
            with db() as conn:
            ...
            await query.edit_message_text("🗑️ Delete Project?\n\nProject ကိုဖျက်မလား?", reply_markup=InlineKeyboardMarkup([[custom_inline_button("🗑️ Confirm Delete", callback_data=f"delete_confirm:{project_id}"), custom_inline_button("⬅️ Cancel", callback_data=f"project:{project_id}")]]))
        elif action == "rename":
            context.user_data["pending_rename"] = {"kind": "project", "project_id": project_id}
            await query.edit_message_text("✏️ Rename\n\nProject name အသစ်ကို ရိုးရိုးပို့ပါ။")
