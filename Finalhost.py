...
def premium_html(text: str) -> str:
    # Message text stays valid plain/HTML text; Premium icons are attached to buttons
    # through icon_custom_emoji_id, preventing raw <tg-emoji> tags from appearing.
    return text
...
async def projects(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not rows:
        if update.callback_query:
            await update.callback_query.edit_message_text("📂 Projects\n\nNo projects found.", reply_markup=menu())
        else:
...
async def create_project_from_pending(update: Update, context: ContextTypes.DEFAULT_TYPE, raw_name: str) -> None:
    ...
    if not pending:
        await update.message.reply_text("📨 အရင်ဆုံး .py, .js, သို့မဟုတ် .zip ဖိုင် ပို့ပါ။")
        return
    project_name = safe_name(raw_name)
    if not raw_name.strip() or project_name != raw_name.strip():
        await update.message.reply_text("⚠️ Project name မှာ English letters, numbers, dot, dash, underscore ပဲသုံးပါ။")
        context.user_data["pending_upload"] = pending
        return
    if project_for(update.effective_user.id, project_name):
        await update.message.reply_text("❌ That project name already exists. နာမည်အသစ်ပို့ပါ။")
        context.user_data["pending_upload"] = pending
        return
    with db() as conn:
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Project Created Successfully!\n\n📁 Name: {project_name}\n\n📦 Type: {kind}\n\n📄 Entrypoint: {entry}", reply_markup=menu())
    except Exception as exc:
        shutil.rmtree(root, ignore_errors=True)
        await update.message.reply_text(f"❌ Upload rejected: {exc}")
...
async def name_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not context.args:
        await update.message.reply_text("📨 ဖိုင်ပို့ပြီး project name ကို ရိုးရိုးပို့ပါ။ Command မလိုပါ။")
        return
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text in ("📦 Dependencies", "Dependencies"):
        await update.message.reply_text("📦 Dependencies\n\nrequirements.txt သို့မဟုတ် package.json ပါတဲ့ project ကို Projects ထဲကရွေးပြီး ▶️ Run နှိပ်ပါ။")
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "request_access":
        ...
        await query.edit_message_text("📨 Add File\n\n.py, .js, သို့မဟုတ် .zip ဖိုင်ကို ပို့ပါ။\n\nဖိုင်ကို ဒီ chat ထဲ ပို့လိုက်ပါ။")
    elif data == "projects":
